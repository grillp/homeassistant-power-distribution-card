...
This is a [home-assistant](home-assistant.io) card for people who have electricity generation and storage in their home. While the  Home assistant Energy Integration contains a great vire to see how much energy (in kWh) has been distributed throughout the day, I was missing the kind of view that shows how the power (in kW) was being distributed around my house right now!
...
- The look and feel are obviously inspired byt the home-assistant card.

- The configuration, additional equipment and customisabilty are inspired by [@reptilex](https://github.com/reptilex)'s card.
...
The lines between the elements represents the flow of Power between elements. When there is Power flowing between elements, a circle moves along the line to represent flow direction and the speed of the ball represents the amount of power, relative to the other flows.

The power that flows between each element is defined in configuration though 9 flow definition. You can define as few or as many flows as you need, depending on your use case, with the only mandatory flow being 'grid to load', as usually that is the least that people have in their home. Sorry to all those that are completely off-grid!

The flows are configured by assigning entities ids to each flow:
...
This is the what is shown above (or below in some cases) they element's 'circle'.
...
- If you use an entity id it will show the entities formatted state
...
>- You will also note that if you change the title of an entity, the graphical editor will also change any prompts that use that entity title to represent what you enter here. ef no value is**doestnot**, once again the default values will be used in the graphical editor.
>- e.g if you entered a value of 'Solar' for the Generation entity title, the prompt for the generation_to_battery flow will change from `Generation → Battery` to `Solar → Battery`. The prompts for Icons and Extra Info will also change to represent the new value.
...
This takes a `string` value (or use the Icon Picker in the visual editor) in the standard icon format of `mdi:<icon name>`
...
The visibility of the `Generation`, `Battery`, `Load Top` and `Load Bottom` elements ant their associated flow's change based on the flows configured.

That is, if an element has no flows configure from or to it, it and it's flows will be hidden.

If you define a single flow from or to an element, the element and all it's flows will be shown.

>**Note:**
>- When you field load add the card to a panel, only the `Grid` and `Load` elements are show, as no flows have been configured
>- Element visibility also affects the fields that are visible in the visual editor for the card.
>- e.g if no flows are defined either into our out of the `Generation` element, the entry fields to set the `Title`, `Icon` and `Extra Info` for the `Generation` element will not be visible. You will first need to configure one or more flows that include the `Generation` element before you can configure that elements additional values
...
2. Add a resource under lovelace (you have to enable advanced Mode in your user profile to see the resource tab) by clicking on the .
    - the URL should be `local\www\power-distrution.js` and Resource Type `javascript`
...
You got a house (load) and you are connected to your local electricity grid. and you have a `sensor` that shows the power (in kW!) coming into your house. You are good to go!
...
# Info (note mix of entities and strings)
grid_info_id: sensor.cbr_general_price
load_info_id: sensor.ww_temperature
storage_info_id: sensor.cbr_charge
...
If you are missing any sensors for the flows, you can always create your own using sensor templates.

Just ensure they:
- provide only positive numeric values for power in the direction of the flow specified (e.g. generation-to-battery will be a postiive number showing how many kWs are being delivered from teh generration element to the battery).
- reuurn values in kW

If you want to be able to select the from the Graphical Editor ensure they are
   - type: _sensor_
   - class: _power_
   - unit of measure: _Kw_
...
I am fortunate enough to have a Tesla PowerWall 2 wuth the new version of the controller.

The Tesla Powerwall integration](https://www.home-assistant.io/integrations/powerwall/) provides enough sensors to show total energy use of all the elements in your system as well as the current power used or produced by each element.

However this plugin needs the power for specific flows and we can use template sensors to create those values from the ones provided by the Tesla Powerwall integration.

>**Note**
>-The below examples use the sensor names provided for my Tesla Powerwall integration configuration. They all start with `sensor.cbr_`. Make sure you change these values to suit your own configuration.
>-The example below **does not** include flow from the grid to battery or or visa versa. In my situation I don't need this as I don't do battery load shifting. A more complete example of template sensors can be found in the [tesla-style-solar-power-card](https://github.com/reptilex/tesla-style-solar-power-card#tesla-powerwall-usage) documentation.
>-You can scrap the `unique_id:` configuration variable. I use them so that I can mve the sensors into an Area.

```yaml
template:
  - sensor:
    power_flow_grid_to_house_entity:
      value_template: "{{ (max([(states('sensor.cbr_site_power')|float), 0.0])) | float}}"
      device_class: power
      unit_of_measurement: kW
    ...
    power_flow_generation_to_grid_entity:
      value_template: "{{ (states('sensor.cbr_site_power') | float * -1) if ((states('sensor.cbr_site_power') | float < 0)) else 0 }}"
      device_class: power
      unit_of_measurement: kW
      unique_id: b12cae24-8b57-11ed-a1eb-034d123543
    power_flow_generation_to_battery_entity:
      value_template: "{{ (states('sensor.cbr_battery_power') | float * -1) if ((states('sensor.cbr_battery_power') | float < 0)) else 0 }}"
      device_class: power
      unit_of_measurement: kW
      unique_id: b12cae24-8b57-11ed-a1eb-044d123543
    power_flow_generation_to_house_entity:
      value_template: "{{ (states('sensor.cbr_solar_power') | float) + (min([states('sensor.cbr_battery_power')|float,0])) + (min([states('sensor.cbr_site_power')|float,0])) }}"
      device_class: power
      unit_of_measurement: kW
      unique_id: b12cae24-8b57-11ed-a1eb-054d123543
    power_flow_battery_to_house_entity:
      value_template: "{{ max([0,states('sensor.cbr_battery_power') | float])  }}"
      device_class: power
      unit_of_measurement: kW
